...
#define in_len 196 // INPUT SIZE
#define epochs 1000 // NUMBER OF epochs
#define val_split 0.2 // PART USED FOR TESTING
#define learn_rate 0.02 // LEARNING COEF.
#define batch_size 1 // MAXIMAL SIZE OF ONE BATCH
#define exit_delta 0
#define exit_buff 10
#define NAME "results/100/1.txt"
//#define lambda 0.0000001 // L2 REG
...
int main(int argc, char **argv){
    ...
    for(int epoch = 0; epoch < epochs; epoch++){
        ...
        if(rank == size){
            ...
            printf("Acc: %.5lf.\n", double(b_tot)/size2);
            err[epoch] = b_ce/size2;
            acc[epoch] = double(b_tot)/size2;
            if(epoch >= exit_buff){
                bool fl = true;
                for(int i = 1; i <= exit_buff; i++){
                    if(err[epoch] <= err[epoch-i] + exit_delta || acc[epoch] >= acc[epoch-i] + exit_delta){
                        fl = false;
                    }
                }
                if(fl){
                    ...
                    fprintf(fp, "%lf", err[epoch]);
                    fprintf(fp, "],\n\"acc\": [");
                    for(int i = 0; i < epoch; i++){
                        fprintf(fp, "%lf,", acc[i]);
                    }
                    fprintf(fp, "%lf", acc[epoch]);
                    fprintf(fp, "]}");
                    fclose(fp);
